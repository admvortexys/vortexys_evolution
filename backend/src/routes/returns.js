/**
 * DevoluÃ§Ãµes: criar devoluÃ§Ã£o vinculada a pedido, processar, creditar cliente.
 * Exige permissÃ£o orders+stock+financial (write).
 */
...
const router = Router();
router.use(auth);
router.use(requirePermission('returns', 'write'));

const RETURN_TYPES = { return_client:'DevoluÃ§Ã£o cliente', exchange:'Troca', warranty:'Garantia', supplier_return:'DevoluÃ§Ã£o fornecedor' };
...
const DESTINATIONS = { available:'Estoque disponÃ­vel', quarantine:'Quarentena', service:'AssistÃªncia/OS', scrap:'Sucata/descarte', supplier_return:'DevoluÃ§Ã£o fornecedor' };
...
// â”€â”€â”€ Find order to return â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ List returns â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Get return detail â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
router.get('/:id', async (req, res, next) => {
  try {
    ...
    if (!r.rows.length) return res.status(404).json({ error: 'DevoluÃ§Ã£o nÃ£o encontrada' });
    const items = await db.query(
...
// â”€â”€â”€ Create return (draft) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
router.post('/', async (req, res, next) => {
  const { order_id, type, origin, items, checklist, refund_type, refund_method, notes } = req.body || {};
  if (!order_id) return res.status(400).json({ error: 'Pedido Ã© obrigatÃ³rio' });
  ...
  try {
    ...
    if (!order.rows.length) return res.status(404).json({ error: 'Pedido nÃ£o encontrado' });
    const ord = order.rows[0];

    if (!['confirmed','separated','delivered'].includes(ord.status)) {
      return res.status(400).json({ error: 'SÃ³ Ã© possÃ­vel devolver pedidos confirmados, separados ou entregues' });
    }
    ...
    for (const it of items) {
      ...
      if (!it.quantity_returned || parseFloat(it.quantity_returned) <= 0) return res.status(400).json({ error: 'Quantidade invÃ¡lida' });

      const oiRow = await conn.query('SELECT * FROM order_items WHERE id=$1 AND order_id=$2', [it.order_item_id, order_id]);
      if (!oiRow.rows.length) return res.status(400).json({ error: `Item ${it.order_item_id} nÃ£o pertence ao pedido` });
      ...
      if (parseFloat(it.quantity_returned) > maxQty + 0.01) {
        return res.status(400).json({ error: `Quantidade mÃ¡xima para devolver deste item: ${maxQty}` });
      }

      if (it.unit_id) {
        const unitCheck = await conn.query('SELECT * FROM product_units WHERE id=$1', [it.unit_id]);
        if (!unitCheck.rows.length) return res.status(400).json({ error: 'Unidade IMEI nÃ£o encontrada' });
        ...
        if (existingReturn.rows.length) return res.status(400).json({ error: `IMEI/Serial jÃ¡ devolvido anteriormente` });
      }
...
// â”€â”€â”€ Change return status â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
router.patch('/:id/status', async (req, res, next) => {
  const { status, notes } = req.body || {};
  if (!status || !STATUSES.includes(status)) return res.status(400).json({ error: 'Status invÃ¡lido' });

  const conn = await db.connect();
  try {
    await conn.query('BEGIN');
    const cur = await conn.query('SELECT * FROM returns WHERE id=$1 FOR UPDATE', [req.params.id]);
    if (!cur.rows.length) return res.status(404).json({ error: 'DevoluÃ§Ã£o nÃ£o encontrada' });
    ...
    if (!TRANSITIONS[ret.status]?.includes(status)) {
      return res.status(400).json({ error: `NÃ£o Ã© possÃ­vel ir de "${ret.status}" para "${status}"` });
    }
        ...
        if (dest === 'available') {
          ...
          if (p.rows.length) {
            ...
            await conn.query(
              ...
              [it.product_id, qty, prev, newQty, `DevoluÃ§Ã£o ${ret.number} â€” estoque disponÃ­vel`, req.params.id, req.user.id, ret.number]
            );
          }
          ...
          if (p.rows.length) {
            ...
            await conn.query(
              ...
              [it.product_id, qty, prev, `DevoluÃ§Ã£o ${ret.number} â€” quarentena`, req.params.id, req.user.id, ret.number]
            );
          }
          ...
          if (p.rows.length) {
            ...
            await conn.query(
              `INSERT INTO stock_movements (product_id,type,quantity,previous_qty,new_qty,reason,reference_id,reference_type,user_id,movement_type,document_type,document_number,qty_in,notes)
               VALUES ($1,'in',$2,$3,$3,$4,$5,'return',$6,'return_client','return',$7,$2,'AssistÃªncia tÃ©cnica')`,
              [it.product_id, qty, prev, `DevoluÃ§Ã£o ${ret.number} â€” assistÃªncia`, req.params.id, req.user.id, ret.number]
            );
          }
          ...
          if (p.rows.length) {
            ...
            await conn.query(
              ...
              [it.product_id, qty, prev, `DevoluÃ§Ã£o ${ret.number} â€” sucata`, req.params.id, req.user.id, ret.number]
            );
          }
          ...
          if (p.rows.length) {
            ...
            await conn.query(
              `INSERT INTO stock_movements (product_id,type,quantity,previous_qty,new_qty,reason,reference_id,reference_type,user_id,movement_type,document_type,document_number,qty_out,notes)
               VALUES ($1,'out',$2,$3,$3,$4,$5,'return',$6,'return_supplier','return',$7,$2,'DevoluÃ§Ã£o ao fornecedor')`,
              [it.product_id, qty, prev, `DevoluÃ§Ã£o ${ret.number} â€” fornecedor`, req.params.id, req.user.id, ret.number]
            );
          }
      ...
      if (ret.refund_type === 'credit' && ret.client_id) {
        ...
        const cr = await conn.query(
          ...
          [creditNum, ret.client_id, ret.order_id, ret.total_refund,
           `DevoluÃ§Ã£o ${ret.number}`, ret.order_number, ret.total_refund, JSON.stringify(itemsJson), req.user.id]
        );
        ...
        if (ret.client_id) {
          await conn.query(
            ...
            [`Estorno devoluÃ§Ã£o ${ret.number}`, ret.total_refund, ret.client_id, ret.order_id,
             `DevoluÃ§Ã£o ${ret.number} â€” ${ret.refund_method || 'estorno'}`, req.user.id]
          );
        }
      }
...
// â”€â”€â”€ Delete draft â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
router.delete('/:id', async (req, res, next) => {
  try {
    const r = await db.query("DELETE FROM returns WHERE id=$1 AND status='draft' RETURNING id", [req.params.id]);
    if (!r.rows.length) return res.status(400).json({ error: 'SÃ³ Ã© possÃ­vel excluir devoluÃ§Ãµes em rascunho' });
